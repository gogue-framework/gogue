...
func GenerateCavern(surface *gamemap.GameMap, wallGlyph, floorGlyph ui.Glyph, smoothingPasses int) {
	...
	for x := 0; x < surface.Width; x++ {
		for y := 0; y < surface.Height; y++ {
			state := rand.Intn(100)
			if state < 30 {
				surface.Tiles[x][y] = &gamemap.Tile{Glyph: wallGlyph, Blocked: true, BlocksSight: true, Visited: false, Explored: false, Visible: false, X: x, Y: y, Noises: make(map[int]float64)}
			} else {
				surface.Tiles[x][y] = &gamemap.Tile{Glyph: floorGlyph, Blocked: false, BlocksSight: false, Visited: false, Explored: false, Visible: false, X: x, Y: y, Noises: make(map[int]float64)}
			}
		}
	}
